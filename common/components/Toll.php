...
class Toll extends Component
{
     ...
     */
    public $authTimeoutParam = '_e_expire';
    /**
    ...
    public function init()
    {
        ...
        if ($this->identityClass === null) {
            throw new InvalidConfigException('Toll::identityClass must be set.');
        }
        if ($this->enableAutoLogin && !isset($this->identityCookie['name'])) {
            throw new InvalidConfigException('Toll::identityCookie must contain the "name" element.');
        }
    }
    ...
    public function login(IdentityInterface $identity, $duration = 0)
    {
        if ($this->beforeLogin($identity, false, $duration)) {
            ...
            if ($this->enableSession) {
                $log = "Toll '$id' logged in from $ip with duration $duration.";
            } else {
                $log = "Toll '$id' logged in from $ip. Session not enabled.";
            }
    ...
    public function logout($destroySession = true)
    {
        ...
        if ($identity !== null && $this->beforeLogout($identity)) {
            ...
            $id = $identity->getId();
            $ip = Yii::$app->getRequest()->getUserIP();
            Yii::info("Toll '$id' logged out from $ip.", __METHOD__);
    ...
    {
        $this->trigger(self::EVENT_AFTER_LOGIN, new TollEvent([
            'identity' => $identity,
            'cookieBased' => $cookieBased,
            'duration' => $duration,
        ]));
    ...
    {
        $this->trigger(self::EVENT_AFTER_LOGOUT, new TollEvent([
            'identity' => $identity,
        ]));
    }
